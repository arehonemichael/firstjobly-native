      ...
      <View style={styles.jobInfo}>
        <View style={styles.companyStatusRow}>
          ...
          <View style={styles.statusPill}>
            <Text style={styles.statusText}>{job.is_urgent ? "Hot" : "New"}</Text>
          </View>
        ...
        <View style={styles.cardFooter}>
          ...
          <View style={styles.postedSalaryStack}>
            <View style={styles.postedPill}>
              <Text style={styles.postedText}>{postedLabel(job)}</Text>
            </View>
...
export default function JobsScreen() {
  ...
  const activeFilters = useMemo(() => {
    ...
    if (filters.minSalary) {
      list.push({ key: "minSalary", label: `R${filters.minSalary.toLocaleString("en-ZA")}+ p/m` });
    }
    if (filters.closingWithin) {
      list.push({ key: "closingWithin", label: `Closing in ${filters.closingWithin} days` });
    }
    return list;
  }, [filters]);
  ...
  function runSearch() {
    setVisibleCount(PAGE_SIZE);
    setFilters((current) => ({ ...current, q: keyword.trim() }));
  }
  ...
  function clearFilters() {
    setDraftFilters({ q: filters.q, sort: filters.sort });
  }
  ...
  return (
    <SafeAreaView style={styles.page} edges={["top", "bottom"]}>
      <FlatList
          ...
          <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.activeFilters}>
                {activeFilters.map((item) => (
                  <TouchableOpacity key={item.key} style={styles.activeChip} onPress={() => removeFilter(item.key)}>
                    ...
                    <Ionicons name="close" size={14} color={theme.colors.inkSoft} />
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
            ...
            <View style={styles.sortRow}>
              {([ ["newest", "Newest"], ["relevance", "Relevance"], ["closing", "Closing"] ] as const).map(([value, label]) => (
                <TouchableOpacity
                  key={value}
                  style={[styles.sortButton, filters.sort === value && styles.sortButtonActive]}
        ...
        ListEmptyComponent={
          <EmptyState title="No opportunities match your filters" message="Try changing your search or removing one or more filters." />
        }
        ListFooterComponent={
          visibleJobs.length < results.length ? (
            <View style={styles.loadMore}>
              <ActivityIndicator color={theme.colors.brand} />
              <Text style={styles.loadMoreText}>Loading more opportunities...</Text>
            </View>
          ) : results.length > 0 ? (
            <Text style={styles.endText}>You've reached all matching opportunities.</Text>
          ) : null
        }
      />
          ...
          <SafeAreaView style={styles.sheet} edges={["bottom"]}>
            <View style={styles.sheetHeader}>
              <View>
                ...
                <Text style={styles.sheetSubtitle}>Narrow down your opportunities</Text>
              </View>
              <TouchableOpacity style={styles.closeButton} onPress={() => setFilterOpen(false)}>
                <Ionicons name="close" size={23} color={theme.colors.ink} />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView contentContainerStyle={styles.sheetContent} showsVerticalScrollIndicator={false}>
              <SelectBlock
                ...
                values={CATEGORIES.map((c) => ({ value: c.key, label: formatCategoryLabel(c.key) }))}
              ...
              <SelectBlock
                ...
                values={PROVINCES.map((province) => ({ value: province, label: province }))}
              ...
              <View style={styles.filterSection}>
                ...
                <TextInput
                  ...
                  onChangeText={(city) => setDraftFilters((current) => ({ ...current, city: city || undefined }))}
              ...
              <SelectBlock
                ...
                values={EXPERIENCE_LEVELS.map((level) => ({ value: level, label: formatCategoryLabel(level) }))}
                onChange={(experience) => setDraftFilters((current) => ({ ...current, experience }))}
              />

              <SelectBlock
                label="Minimum education"
                value={draftFilters.education}
                values={EDUCATION_LEVELS.map((level) => ({ value: level, label: formatCategoryLabel(level) }))}
                onChange={(education) => setDraftFilters((current) => ({ ...current, education }))}
              />

              <View style={styles.filterSection}>
                <Text style={styles.filterLabel}>
                  Minimum salary: R{(draftFilters.minSalary ?? 0).toLocaleString("en-ZA")} p/m
                </Text>
                <View style={styles.salaryOptions}>
                  {[0, 5000, 10000, 15000, 20000, 30000, 40000].map((amount) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={amount}
                        style={[styles.salaryOption, selected && styles.salaryOptionSelected]}
                        onPress={() => setDraftFilters((current) => ({ ...current, minSalary: amount === 0 ? undefined : amount }))}
                      >
                        <Text style={[styles.salaryOptionText, selected && styles.salaryOptionTextSelected]}>
                          {amount === 0 ? "Any salary" : `R${amount.toLocaleString("en-ZA")}+`}
                        </Text>
                      </TouchableOpacity>
                    );
                  })}
              ...
              <SelectBlock
                label="Closing date"
                value={draftFilters.closingWithin ? String(draftFilters.closingWithin) : undefined}
                values={[
                  { value: "7", label: "Within 7 days" },
                  { value: "14", label: "Within 14 days" },
                  { value: "30", label: "Within 30 days" },
                ]}
                onChange={(value) => setDraftFilters((current) => ({ ...current, closingWithin: value ? Number(value) : undefined }))}
              />
            ...
            <View style={styles.sheetFooter}>
              <TouchableOpacity style={styles.applyFiltersButton} onPress={applyFilters}>
                <Text style={styles.applyFiltersText}>Show matching jobs</Text>
              </TouchableOpacity>
            </View>
          </SafeAreaView>
        </View>
