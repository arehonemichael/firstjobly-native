...
function closingCountdown(job: Job): string | null {
  ...
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(job.closing_date).slice(0, 10));
...
function jobPath(job: Job) { return { pathname: "/jobs/[id]" as const, params: { id: job.id } }; }

function johannesburgGreeting() {
  try {
    const hourPart = new Intl.DateTimeFormat("en-ZA", { hour: "2-digit", hour12: false, timeZone: "Africa/Johannesburg" }).formatToParts(new Date()).find((part) => part.type === "hour")?.value;
    ...
    <TouchableOpacity activeOpacity={0.82} style={styles.topCard} onPress={() => router.push(jobPath(item))}>
      <View style={styles.topCardHead}>
        <View style={styles.companyMark}>
          {item.company_logo_url ? <ExpoImage source={{ uri: item.company_logo_url }} style={styles.companyLogoImage} contentFit="contain" cachePolicy="memory-disk" transition={0} /> : <Text style={styles.companyMarkText}>{item.company_name.charAt(0).toUpperCase()}</Text>}
        </View>
      ...
      {closing ? <Text numberOfLines={1} style={styles.closingCountdownText}>{closing}</Text> : null}
      {salary ? <Text numberOfLines={1} style={styles.compactSalaryText}>{salary}</Text> : null}
      <View style={styles.locationRow}><Ionicons name="location-outline" size={14} color={theme.colors.inkSoft} /><Text numberOfLines={1} style={styles.locationText}>{formatLocation(item)}</Text></View>
    </TouchableOpacity>
  );
});
  ...
  return (
    <TouchableOpacity activeOpacity={0.84} style={styles.matchCard} onPress={() => router.push(jobPath(item))}>
      <View style={styles.matchLogo}>
        {item.company_logo_url ? <ExpoImage source={{ uri: item.company_logo_url }} style={styles.matchLogoImage} contentFit="contain" cachePolicy="memory-disk" transition={0} /> : <Text style={styles.matchLogoText}>{item.company_name.charAt(0).toUpperCase()}</Text>}
      </View>
      <View style={styles.matchBody}>
        ...
        <View style={styles.matchFooter}>
          <View style={styles.tagsRow}>
            {tagsFor(item).map((tag, index) => <View key={`${item.id}-${tag}-${index}`} style={styles.tagPill}><Text numberOfLines={1} style={styles.tagText}>{tag}</Text></View>)}
          </View>
          <View style={styles.postedSalaryStack}>
            <View style={styles.postedPill}><Text style={styles.postedText}>{postedLabel(item)}</Text></View>
            {closing ? <Text numberOfLines={1} style={styles.closingCountdownText}>{closing}</Text> : null}
            {salary ? <Text numberOfLines={1} style={styles.compactSalaryText}>{salary}</Text> : null}
          </View>
        </View>
...
function TopOpportunitySkeleton() {
  return <View style={[styles.topCard, styles.skeletonCard]}><View style={styles.topCardHead}><View style={[styles.companyMark, styles.skeletonBlock]} /><View style={styles.skeletonTopCopy}><View style={[styles.skeletonLine, styles.skeletonLineMedium]} /><View style={[styles.skeletonLine, styles.skeletonLineShort]} /></View></View><View style={[styles.skeletonLine, styles.skeletonTitleLine]} /><View style={[styles.skeletonLine, styles.skeletonLineShort]} /><View style={[styles.skeletonLine, styles.skeletonLineMedium]} /></View>;
}

function MatchSkeleton() {
  return <View style={[styles.matchCard, styles.skeletonCard]}><View style={[styles.matchLogo, styles.skeletonBlock]} /><View style={styles.matchBody}><View style={[styles.skeletonLine, styles.skeletonLineMedium]} /><View style={[styles.skeletonLine, styles.skeletonTitleLine]} /><View style={[styles.skeletonLine, styles.skeletonLineShort]} /><View style={[styles.skeletonLine, styles.skeletonWideLine]} /><View style={styles.skeletonFooterRow}><View style={[styles.skeletonPill, styles.skeletonBlock]} /><View style={[styles.skeletonPill, styles.skeletonBlock]} /></View></View></View>;
}
...
export default function HomeScreen() {
  ...
  const jobsRoute = useMemo(() => ({ pathname: "/jobs" as const, params: Object.fromEntries(Object.entries(filter).filter(([, value]) => Boolean(value))) }), [filter]);
  const firstName = useMemo(() => {
    const metadata = user?.user_metadata ?? {};
    const value = String(metadata.first_name ?? metadata.full_name ?? metadata.name ?? "").trim();
  ...
  const loadRecommendations = useCallback(async (force = false) => {
    ...
    try {
      const [topResult, personalizedResult] = await Promise.allSettled([
        withTimeout("Home top opportunities", getTopOpportunities(userId, filter, 10)),
        withTimeout("Home personalized matches", getPersonalizedMatches(userId, filter, 20)),
      ]);
  ...
  }, [authLoading, userId, filter, loadRecommendations]);

  useEffect(() => {
    const subscription = AppState.addEventListener("change", (state) => {
      if (state === "active" && isFocused) void loadRecommendations(false);
    });
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingBottom: bottomContentPadding + 18 }]} showsVerticalScrollIndicator={false}>
        <View style={styles.topBar}>
          <View style={styles.greetingBlock}><Text style={styles.greetingText}>{greeting}</Text><Text style={styles.homeLabel}>Find your next opportunity</Text></View>
          <TouchableOpacity activeOpacity={0.78} style={styles.notificationButton}><Ionicons name="notifications-outline" size={23} color={theme.colors.ink} /><View style={styles.notificationDot} /></TouchableOpacity>
        </View>
        ...
        <View style={styles.heroWrap}>
          <ImageBackground key={`hero-${heroImageIndex}`} source={HERO_IMAGES[heroImageIndex]} resizeMode="cover" style={styles.heroImage} imageStyle={styles.heroImageRadius}>
            ...
            <View style={styles.heroContent}>
              ...
              <TouchableOpacity activeOpacity={0.86} style={styles.heroCta} onPress={() => router.push(jobsRoute)}><Text style={styles.heroCtaText}>See recommendations</Text><Ionicons name="arrow-forward" size={20} color={theme.colors.ink} /></TouchableOpacity>
            </View>
            <View style={styles.carouselDots}>{HERO_IMAGES.map((_, index) => <View key={index} style={[styles.carouselDot, index === heroImageIndex && styles.carouselDotActive]} />)}</View>
          </ImageBackground>
        </View>

        <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>Top opportunities near you</Text><TouchableOpacity onPress={() => router.push(jobsRoute)}><Text style={styles.seeAll}>See all</Text></TouchableOpacity></View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.topCardsRow}>
          {recommendationsLoading && topOpportunities.length === 0 ? [0, 1, 2].map((key) => <TopOpportunitySkeleton key={key} />) : topOpportunities.map((item) => <TopOpportunityCard key={item.id} item={item} />)}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  topBar: { minHeight: 62, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  greetingBlock: { flex: 1, minWidth: 0, paddingRight: 16 },
  greetingText: { color: theme.colors.inkSoft, fontSize: 13, lineHeight: 18, fontWeight: "600" },
  homeLabel: { color: theme.colors.ink, fontSize: 18, lineHeight: 24, fontWeight: "700", marginTop: 1 },
  notificationButton: { width: 46, height: 46, borderRadius: theme.radius.pill, backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.line, alignItems: "center", justifyContent: "center", ...theme.shadow.card },
  notificationDot: { position: "absolute", width: 10, height: 10, borderRadius: 5, backgroundColor: theme.colors.brand, right: 3, top: 3, borderWidth: 2, borderColor: theme.colors.surface },
  ...
  heroHeadline: { color: theme.colors.primaryForeground, fontSize: 31, lineHeight: 36, fontWeight: "800", letterSpacing: -0.6 },
  heroSubtext: { color: theme.colors.primaryForeground, fontSize: 15, lineHeight: 21, fontWeight: "500", marginTop: 12, maxWidth: 300 },
  heroCta: { alignSelf: "flex-start", height: 48, paddingHorizontal: 18, marginTop: 20, borderRadius: theme.radius.pill, backgroundColor: theme.colors.surface, flexDirection: "row", alignItems: "center", gap: 10 },
  heroCtaText: { color: theme.colors.ink, fontSize: 14, fontWeight: "700" },
  carouselDots: { position: "absolute", right: 18, bottom: 16, flexDirection: "row", gap: 8 },
  carouselDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: theme.colors.surface, opacity: 0.92 },
  carouselDotActive: { backgroundColor: theme.colors.brand, opacity: 1 },
  sectionHeader: { paddingHorizontal: 16, marginTop: 22, marginBottom: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 },
  matchesHeader: { marginTop: 26 },
  sectionTitle: { flexShrink: 1, color: theme.colors.ink, fontSize: 20, lineHeight: 26, fontWeight: "700", letterSpacing: -0.25 },
  seeAll: { color: theme.colors.brand, fontSize: 14, fontWeight: "600" },
  topCardsRow: { paddingLeft: 16, paddingRight: 8, gap: 10 },
  topCard: { width: 190, minHeight: 208, backgroundColor: theme.colors.surface, borderRadius: theme.radius.md, borderWidth: 1, borderColor: theme.colors.line, padding: 12, ...theme.shadow.card },
  topCardHead: { flexDirection: "row", alignItems: "flex-start", gap: 10 },
  companyMark: { width: 44, height: 44, borderRadius: theme.radius.sm, backgroundColor: theme.colors.brandSoft, alignItems: "center", justifyContent: "center", overflow: "hidden" },
  companyLogoImage: { width: 36, height: 36 },
  companyMarkText: { color: theme.colors.brand, fontSize: 20, fontWeight: "800" },
  ...
  compactSalaryText: { flexShrink: 1, color: theme.colors.brand, fontSize: 10, lineHeight: 14, fontWeight: "600", marginTop: 3 },
  locationRow: { marginTop: 9, flexDirection: "row", alignItems: "center", gap: 4 },
  locationText: { flex: 1, color: theme.colors.inkSoft, fontSize: 12, lineHeight: 16 },
  matchesList: { paddingHorizontal: 16, gap: 10 },
  matchCard: { minHeight: 170, backgroundColor: theme.colors.surface, borderRadius: theme.radius.md, borderWidth: 1, borderColor: theme.colors.line, padding: 12, flexDirection: "row", gap: 12, ...theme.shadow.card },
  matchLogo: { width: 56, height: 56, borderRadius: theme.radius.sm, backgroundColor: theme.colors.brandSoft, alignItems: "center", justifyContent: "center", overflow: "hidden" },
  matchLogoImage: { width: 46, height: 46 },
  matchLogoText: { color: theme.colors.brand, fontSize: 24, fontWeight: "800" },
  ...
  matchFooter: { marginTop: 11, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 8 },
  ...
  skeletonFooterRow: { flexDirection: "row", gap: 8, marginTop: 20 },
  skeletonPill: { width: 62, height: 24, borderRadius: 7 },
});