...
function johannesburgGreeting() {
  try {
    const hourPart = new Intl.DateTimeFormat("en-ZA", {
      hour: "2-digit", hour12: false, timeZone: "Africa/Johannesburg",
...
export default function HomeScreen() {
  ...
  const jobsRoute = useMemo(() => ({
    ...
    params: Object.fromEntries(Object.entries(filter).filter(([, value]) => Boolean(value))),
  }), [filter]);
  ...
  useEffect(() => {
    // Warm the other hero assets without changing the visible source. This keeps
    // reopening Home smooth while avoiding a mid-view remote image replacement.
    HERO_IMAGES.forEach((image) => { void Image.prefetch(image.uri); });
  }, []);
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingBottom: bottomContentPadding + 18 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroWrap}>
          <ImageBackground source={HERO_IMAGES[heroImageIndex]} resizeMode="cover" style={styles.heroImage} imageStyle={styles.heroImageRadius}>
            ...
            <View style={styles.heroContent}>
              <View style={styles.recommendedPill}>
                ...
                <Text style={styles.recommendedText}>{firstName ? `Recommended for ${firstName}` : "Recommended for you"}</Text>
              </View>
              <Text style={styles.heroHeadline}>{heroHeadline}</Text>
              <Text style={styles.heroSubtext}>{heroFact}</Text>
              <TouchableOpacity activeOpacity={0.86} style={styles.heroCta} onPress={() => router.push(jobsRoute)}>
                <Text style={styles.heroCtaText}>See recommendations</Text>
                <Ionicons name="arrow-forward" size={20} color={theme.colors.ink} />
              </TouchableOpacity>
            </View>
            <View style={styles.carouselDots}>
              {HERO_IMAGES.map((_, index) => <View key={index} style={[styles.carouselDot, index === heroImageIndex && styles.carouselDotActive]} />)}
            </View>
          </ImageBackground>
        </View>
        ...
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity onPress={() => router.push(jobsRoute)}><Text style={styles.seeAll}>See all</Text></TouchableOpacity>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.topCardsRow}>
          {topOpportunities.map((item) => {
            const salary = formatSalary(item); const closing = closingCountdown(item);
            return (
              <TouchableOpacity key={item.id} activeOpacity={0.82} style={styles.topCard} onPress={() => router.push(jobPath(item))}>
                <View style={styles.topCardHead}>
                  <View style={styles.companyMark}>{item.company_logo_url ? <Image source={{ uri: item.company_logo_url }} style={styles.companyLogoImage} resizeMode="contain" /> : <Text style={styles.companyMarkText}>{item.company_name.charAt(0).toUpperCase()}</Text>}</View>
                  <View style={styles.topCompanyCopy}>
                    <View style={styles.companyNameRow}><Text numberOfLines={1} style={styles.companyName}>{item.company_name}</Text><View style={styles.microBadge}><Text style={styles.microBadgeText}>{item.is_featured ? "★" : "✦"}</Text></View></View>
        ...
        <View style={styles.matchesList}>
          {matches.map((item) => {
            const salary = formatSalary(item); const closing = closingCountdown(item);
            return (
              <TouchableOpacity key={item.id} activeOpacity={0.84} style={styles.matchCard} onPress={() => router.push(jobPath(item))}>
                <View style={styles.matchLogo}>{item.company_logo_url ? <Image source={{ uri: item.company_logo_url }} style={styles.matchLogoImage} resizeMode="contain" /> : <Text style={styles.matchLogoText}>{item.company_name.charAt(0).toUpperCase()}</Text>}</View>
                <View style={styles.matchBody}>
                  <View style={styles.companyStatusRow}><Text numberOfLines={1} style={styles.matchCompany}>{item.company_name}</Text><View style={styles.statusPill}><Text style={styles.statusText}>{item.is_urgent ? "Hot" : "New"}</Text></View><TouchableOpacity style={styles.bookmarkButton} activeOpacity={0.7}><Ionicons name="bookmark-outline" size={22} color={theme.colors.ink} /></TouchableOpacity></View>
                  ...
                  <View style={styles.matchFooter}>
                    <View style={styles.tagsRow}>{tagsFor(item).map((tag) => <View key={tag} style={styles.tagPill}><Text numberOfLines={1} style={styles.tagText}>{tag}</Text></View>)}</View>
                    <View style={styles.postedSalaryStack}><View style={styles.postedPill}><Text style={styles.postedText}>{postedLabel(item)}</Text></View>{closing ? <Text numberOfLines={1} style={styles.closingCountdownText}>{closing}</Text> : null}{salary ? <Text numberOfLines={1} style={styles.compactSalaryText}>{salary}</Text> : null}</View>
                  </View>
                </View>
              </TouchableOpacity>
            );
...
const styles = StyleSheet.create({
  ...
  heroContent: { paddingHorizontal: 20, paddingTop: 22, paddingBottom: 28, maxWidth: 350 },
